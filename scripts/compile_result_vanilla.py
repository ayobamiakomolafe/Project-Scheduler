...
def compute_role_accuracy(data, similarity_model, ROLE_CATALOG):
    role_scores = []
    ...
    for task in data["tasks"]:
        ...
        # ---- Get role title ----
        roles = [ROLE_CATALOG.get(i, None) for i in role_code]
...
ROLE_CATALOG = {
    "ARCH-PRIN-01": "Principal Cloud Architect",
    "ARCH-SR-02": "Senior Software Architect",

    "DATA-LEAD-01": "Lead Data Engineer",
    "ENG-SR-BACK": "Senior Backend Engineer",
    "ENG-MID-BACK": "Mid-Level Backend Engineer",

    "ENG-SR-MOB": "Senior Mobile Applications Engineer",
    "ENG-MID-FRONT": "Frontend Developer (Mid-Level)",

    "OPS-SR-SEC": "Senior DevOps & Security Engineer",
    "QA-AUTO-03": "Automated QA Engineer"
}
...
def parse_and_grade_file(filepath, project_key):
    ...
    for task in data['tasks']:
        try:
            total_agent_cost += float(task['cost in USD'])
        except Exception:
            continue
    try:
    ...
    role_accuracy = compute_role_accuracy(data, similarity_model, ROLE_CATALOG)
    ...
    durations = [float(task['duration in working days']) for task in data['tasks']]
...
def compile_all_results():
    ...
    configurations = {
        "Vanilla Baseline": "vanilla_outputs",
